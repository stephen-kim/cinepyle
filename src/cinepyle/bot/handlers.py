...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        "안녕하세요! cinePyle 영화봇입니다 🎬\n\n"
        "사용 가능한 명령어:\n"
        "/ranking - 오늘의 박스오피스 순위\n"
        "/nearby - 근처 영화관 찾기 + 상영 스케줄\n"
        "/book - 영화 예매\n"
        "/help - 도움말\n\n"
        "💬 자연어로도 예매할 수 있어요!\n"
        '예: "CGV 용산에서 캡틴 아메리카 7시 예매해줘"\n\n'
        "📍 위치를 보내면 근처 영화관과 상영 스케줄을 알려드려요.\n"
        "예매 완료 후 네이버 지도 길찾기 링크도 받을 수 있어요!"
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        "📍 위치 기반 기능:\n"
        "- 위치를 전송하면 근처 영화관과 상영 스케줄을 알려드립니다.\n"
        '- "근처 영화관", "가까운 영화관" 등으로 요청할 수도 있어요.\n'
        "- 예매 완료 시 네이버 지도 길찾기 링크를 보내드립니다.\n\n"
        "🔔 자동 알림:\n"
        "- 매일 아침 9시 영화 다이제스트 (개봉 예정 + 박스오피스 + 기대평 + 씨네21/네이버 링크)\n"
...
async def nearby_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    The actual location processing is done by location_booking_handler
    in bot/booking.py, which finds nearby theaters and shows schedules
    via the LLM agent.
    """
