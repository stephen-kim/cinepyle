...
TOOL_DEFINITIONS: list[dict] = [
    ...
    {
        "name": "movie_info",
        "description": "영화 정보 조회 (감독, 출연진, 장르, 러닝타임, 평점, 리뷰, 관객수, 예매율, OST, 관람등급, 재밌는지 등). 영화에 대한 메타 정보나 평가를 물어보면 이것.",
        "parameters": {
            "reply": {"type": "string", "description": "친근한 한국어 안내 메시지"},
...
- "인터스텔라 재밌어?" / "평점 어때" / "관객수 얼마야" / "OST 뭐야" → movie_info (영화 평가/메타정보)
...
- "영화 보고싶다" / "영화나 볼까" + 지역/시간/영화제목 없음 → new_movies
- "영화나 볼까" / "보러갈까" + 영화 제목 → showtime (영화 제목이 있으면 상영시간 조회!)
- "애들 데리고" / "아이랑 볼만한" / "데이트 영화" → showtime
- "재밌어?" / "평점" / "관객수" / "OST" / "예매율" + 영화 제목 → movie_info (chat이 아님!)
...
_SHOWTIME_KEYWORDS = ["상영시간", "시간표", "몇시", "뭐해", "뭐하", "상영 중", "언제까지", "아직 상영", "상영 기간", "아직 해", "보러갈까", "보러가"]
...
_MOVIE_INFO_KEYWORDS = ["누가 나와", "출연", "감독", "러닝타임", "줄거리", "장르", "영화 정보", "누가 나오",
                        "평점", "재밌", "재미있", "관객수", "관객 수", "예매율", "ost", "관람등급", "관람 등급",
                        "몇분짜리", "리뷰", "후기", "별점"]
